...
if ! heroku buildpacks -a $HEROKU_APP | grep "http://github.com/ericmend/null-buildpack.git" > /dev/null; then
    echo "Setting buildpack"
    heroku buildpacks:set http://github.com/ericmend/null-buildpack.git -a $HEROKU_APP
fi
